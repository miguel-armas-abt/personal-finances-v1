...
import io.github.miguelarmasabt.personal.finances.expenses.rest.server.beans.ExpenseCategoryAssignmentRequestDto;
import io.github.miguelarmasabt.personal.finances.expenses.rest.server.beans.ExpenseCategoryRequestDto;
import io.github.miguelarmasabt.personal.finances.expenses.rest.server.beans.ExpenseCategoryResponseDto;
import io.github.miguelarmasabt.personal.finances.expenses.rest.server.beans.ExpenseSaveRequestDto;
import io.github.miguelarmasabt.personal.finances.expenses.rest.server.beans.ExpenseSearchResponseDto;
import io.github.miguelarmasabt.personal.finances.expenses.rest.server.beans.ExpenseUpdateRequestDto;
...
public class ExpenseRestService implements ExpensesResource {
  ...
  private final ExpenseRestMapper restMapper;
  ...
  public CompletionStage<ExpenseSearchResponseDto> searchExpenses(
    ...
    String userCode = authenticatedUserProvider.userCode();
  ...
  public CompletionStage<Response> saveExpense(
    ...
    String userCode = authenticatedUserProvider.userCode();
    return Uni.createFrom().item(saveRequest)
        .map(restMapper::toDto)
        .flatMap(request -> expenseService.saveExpense(userCode, request)
  ...
  public CompletionStage<Void> updateExpense(
    ...
    String userCode = authenticatedUserProvider.userCode();
    return Uni.createFrom().item(updateRequest)
        .map(restMapper::toDto)
        .flatMap(request -> expenseService.updateExpense(userCode, expenseId, request)
            .replaceWithVoid())
        .subscribeAsCompletionStage();
  }
  ...
  public CompletionStage<Response> assignCategories(
    ...
    String userCode = authenticatedUserProvider.userCode();
    return Uni.createFrom().item(assignmentRequest)
        .map(restMapper::toDto)
        .flatMap(request -> assignmentService.assignCategories(userCode, request)
  ...
  public Uni<Response> importCsv(@RestForm("file") FileUpload file) {
    return importExpenseCsvService.importCsv(authenticatedUserProvider.userCode(), file)
  ...
  public CompletionStage<ExpenseCategoryResponseDto> findAllCategories(
    ...
    return categoryService.findAllCategories(authenticatedUserProvider.userCode())
        .subscribeAsCompletionStage();
  }
  ...
  public CompletionStage<Void> updateExpenseCategories(
    ...
    return categoryService.updateCategories(authenticatedUserProvider.userCode(), restMapper.toDto(updateCategoryRequest))
        .replaceWithVoid()
        .subscribeAsCompletionStage();
  }
}
