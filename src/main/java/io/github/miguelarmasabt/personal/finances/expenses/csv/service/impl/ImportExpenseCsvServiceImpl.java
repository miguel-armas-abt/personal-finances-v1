package io.github.miguelarmasabt.personal.finances.expenses.csv.service.impl;
...
import io.github.miguelarmasabt.validations.BodyConstraintValidator;
...
public class ImportExpenseCsvServiceImpl implements ImportExpenseCsvService {
  ...
  private final BodyConstraintValidator validator;
  ...
  public Uni<Void> importCsv(String userCode, FileUpload file) {
    return Uni.createFrom()
        ...
        .onItem().transformToUniAndConcatenate(validator::validateAndGet)
  ...
  private Uni<Void> persistAllOrNothingInTransaction(List<ExpenseEntity> entities) {
    ...
    return Panache.withTransaction(() ->
        Multi.createFrom().iterable(entities)
            .group().intoLists().of(properties.technical().csv().imports().batchSize())
